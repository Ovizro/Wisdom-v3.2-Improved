...
void material_build(
	out Material mat, 
	in vec3 fpos, in vec3 vpos, in vec3 wpos, in vec3 N,
...
void material_sample(out Material mat, in vec2 uv) {
	vec3 fpos = vec3(uv, texture2D(depthtex1, uv).x);
	vec4 vpos = fetch_vpos(fpos);
	vec3 normal = normalDecode(texture2D(gnormal, uv).rg);
	vec4 spec = texture2D(gaux1, uv);
	vec4 color = texture2D(colortex0, uv);
	material_build(
		mat,
		fpos, vpos.xyz, (gbufferModelViewInverse * vpos).xyz, normal,
...
void material_sample_water(out Material mat, in vec2 uv) {
	vec3 fpos = vec3(uv, texture2D(depthtex0, uv).x);
	vec4 vpos = fetch_vpos(fpos);
